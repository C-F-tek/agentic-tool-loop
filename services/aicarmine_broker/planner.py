...
def _single_embedded_json_decision(text: str) -> dict[str, Any]:
    ...
    This is deterministic stripping, not plaintext intent recovery: prose is
    ignored only when the planner also emitted exactly one complete JSON object
    that already contains an ``action`` field. Malformed JSON, multiple JSON
    decisions, or snippets without planner shape remain invalid and continue to
    the existing retry/repair paths.
    """
    raw = str(text or "")
    if not raw.strip():
        return {}
    ...
    for match in re.finditer(r"\{", raw):
        ...
        if action in {"tool", "final", "done", "complete", "completed", "block", "blocked", "need_user", "needs_user"}:
...
def _native_tool_calls_decision(tool_calls: list[dict[str, Any]], raw_text: str = "") -> dict[str, Any]:
    ...
    from .tool_contract import normalize_tool_name, parse_tool_call  # noqa: PLC0415
