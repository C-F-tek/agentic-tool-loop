...
- Before the first planner turn, 3572 may ask 11434 for a bounded
  preplanner RAG query plan. Malformed JSON is repaired through the same
  planner model; timeout/unavailable results are typed diagnostics and can only
  fall back to deterministic preseed, not to hidden planning.
- Repo-analysis and semantic-audit final candidates go through the
  final-quality judge before acceptance. Judge output can accept, reject or
  request a concrete next route, but validator/finalization remains
  authoritative.
- Selected validator rejections can call the planner replan specialist. The
  specialist converts rejection evidence into `required_next_progress` and an
  optional `required_next_tool_call`; malformed specialist JSON is repaired
...
- Public evidence materialization is pointer-first: complete payloads are
  canonical under `tool_context_for_30b.artifacts[*].artifact`, while
  `priority_evidence_for_30b` carries bounded metadata/hash/location and
  `payload_index_for_30b` points to the concrete inline field.
...
| `planner.py` | Main controlled agentic loop. It builds planner prompts, records turns, calls Ollama, validates decisions, wires preplanner/final-quality/replan-specialist model guidance, separates code-product from apply intent, executes internal tools, handles explicit repair routing, writes events/state and finalizes jobs. Highest-risk file in this package. |
| `application/controller/rag_preseed.py` | Controller preseed and preplanner query-plan owner. It asks 11434 for bounded RAG query/path intent, repairs malformed query-plan JSON, and records typed non-blocking fallback diagnostics when the planner backend is unavailable. |
| `application/evidence/final_quality.py` | Evidence-owned final-quality checks and judge request builder for repo-analysis/semantic-audit finals. It defines the bounded judge payload and route vocabulary; `planner.py` performs the 11434 call/repair and the validator remains authoritative. |
| `application/evidence/required_working_set.py` | Required working-set builder for planner prompt content. It rehydrates concrete file/diff/tool-result windows and stores oversized sections through injected prompt-window storage without replacing required evidence with local paths. |
| `application/prompt/pack_builder.py` | Measured 11434 prompt-pack builder. It owns serialized prompt budget reports, optional-context omission/windowing and available-tool windows, but does not call Ollama or mutate validation policy. |
...
`C:\Users\sanit\agentic-tool-loop`.
...
| `application/command/execution_policy.py` | Diagnostic-only command execution policy. It combines command class, cwd, repo root, approval mode and consent into an inspectable JSON policy decision without executing commands or changing dispatch permissions. |
| `application/memory/conflict_detector.py` | Diagnostic-only planner memory conflict detector. It marks wrong-branch, missing-path, out-of-repo and file-hash-mismatch records as ignored/conflicting without deleting SQLite rows or changing planner memory surfaces. |
| `application/npu_phi/client.py` | Best-effort diagnostic client for the external Phi-3.5 NPU sidecar. It can enqueue scene/spec jobs with a tight timeout and no retry, but it is disabled unless `ENABLE_NPU_PHI_BROKER_DIAGNOSTICS=1` and must not block planner validation or finalization. |
| `application/npu_phi/policy.py` | Diagnostic-only policy gate for NPU Phi attempts. It restricts enqueue attempts to visual/scene/music/Blender-style goals and returns `not_applicable` without HTTP for ordinary repo analysis/refactor goals. |
| `application/runtime_debug/debug_packet.py` | Diagnostic-only runtime packet builder for planner/controller rejection paths. It summarizes decision, validator result, evidence coverage, required progress and candidate previews with bounded redaction, without changing dispatch or finalization gates. |
| `application/replay/loop_replay.py` | Offline diagnostic replay harness for persisted agent jobs. It reads job state, events, planner streams and tool-result counts, recomputes evidence/validator diagnostics through injected or facade callbacks and reports loops/mismatches without executing tools or mutating state. |
| `application/search/search_quality.py` | Diagnostic-only search result quality assessor. It turns `ok`, truncation, unreadable file counts and zero-result signals into `complete`, `partial`, `weak` or `failed` guidance without changing planner gates or re-running search. |
...
| `application/evidence/coverage_scorer.py` | Diagnostic-only evidence coverage scorer. It summarizes file, directory, repository and code-product coverage into `evidence_coverage` without changing finalization gates: `coverage_score_ready` can reflect score, while `final_ready` follows only the finalization contract / `planner_may_choose_final`. |
...
| `application/public_payload/payload_index_resolver.py` | Pure resolver for broker public payload-index paths. It verifies locations such as `tool_context_for_30b.artifacts[0].artifact.content` and `tool_context_for_30b.artifacts[0].artifact.unified_diff` against inline JSON so materialization reports can distinguish resolved, missing and empty targets. |
...
| `application/public_payload/openwebui_terminal_answer.py` | Builds the terminal `answer_for_30b` text and `next_action_for_30b` instruction stored inside the structured `tool_context_for_30b` payload. These are broker context fields, not public 3571 top-level fields. |
...
| `application/job/worker.py` | Background job worker application service. It owns running state transitions, planner handoff and disabled-planner legacy one-shot execution through injected persistence/planner/agent dependencies. Planner exceptions are terminalized through the injected finalizer so `failed` jobs keep the same inline `tool_context_for_30b` contract as other terminal states; raw traceback paths remain local operator diagnostics. |
...
| `application/planner/required_progress.py` | Structured companion model for legacy `required_next_progress` text. It exposes explicit factories for known progress kinds and keeps text parsing as fallback, preserving the human text and existing validator behavior. |
...
| `application/tool_surface/action_proof_ledger.py` | Stable proof metadata for planner `candidate_next_actions`. It assigns deterministic `action_id` values and proof fields such as source, path existence/readability, scope and validator admissibility. |
...
| `planner_core/json_io.py` | Ollama HTTP and streaming JSON I/O utilities plus strict JSON object parsing. It guards both response-header wait and stream-frame reads, emits typed stream timeout diagnostics, and observes model turn completion while controller validation remains in `planner.py`. |
