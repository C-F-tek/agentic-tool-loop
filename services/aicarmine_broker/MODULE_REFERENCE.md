...
| `application/code_product_history.py` | Code-product build-state history and action helper. It detects duplicate scratchpad writes, extracts ready/window-only state from planner history, builds exact read/write/propose actions and receives artifact/content rehydration callbacks from the planner. |
