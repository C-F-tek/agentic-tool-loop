...
def deterministic_public_wrapper(*, public_tool_name: str, original_args: dict[str, Any], internal_tool: str, internal_args: dict[str, Any], dispatcher_result: dict[str, Any], selector_response: dict[str, Any], root: Path) -> dict[str, Any]:
    ...
    return {'ok': ok, 'service': 'vulkan_agent', 'mode': V6_MARKER, 'verdict': 'PUBLIC_TOOL_X_RESULT_READY' if ok else 'PUBLIC_TOOL_X_RESULT_FAILED', 'tool_name': public_tool_name, 'tool_result_for': public_tool_name, 'operation_id': public_tool_name, 'called_by_30b': public_tool_name, 'arguments_from_30b': original_args, 'result': dispatcher_result, 'evidence_guide_for_30b': answer, 'tool_context_for_30b': tool_context, 'verified_problems': dispatcher_result.get('verified_problems') if isinstance(dispatcher_result.get('verified_problems'), list) else [], 'useful_next_calls': dispatcher_result.get('useful_next_calls') if isinstance(dispatcher_result.get('useful_next_calls'), list) else [], 'wrapper_call_contract': dispatcher_result.get('wrapper_call_contract') or {'public_tool': public_tool_name, 'rule': 'Call the public wrapper tool again with specific parameters when more local context is needed.'}, 'session_id': root.name, 'workspace': str(root), 'artifacts': artifacts, 'dispatcher_tool_result_l': dispatcher_result, 'openwebui_usage': {'evidence_guide_field': 'evidence_guide_for_30b', 'structured_context_field': 'tool_context_for_30b'}, 'wrapper_contract': {'type': 'deterministic_field_mapping', 'public_tool_x': public_tool_name, 'internal_tool_l': internal_tool, 'mapping': {'tool_name': 'public_tool_x', 'tool_result_for': 'public_tool_x', 'called_by_30b': 'public_tool_x', 'arguments_from_30b': 'original public arguments', 'result': 'raw/structured dispatcher result L', 'evidence_guide_for_30b': 'single global narrative guide', 'ok': 'dispatcher result ok'}}, 'internal_vulkan': {'public_tool_x': public_tool_name, 'pipeline': '3571 -> 3572 -> 11435(select L) -> 3572(dispatch L + deterministic wrap X) -> 3571 -> 30B', 'selector_backend_tool_call': selector_backend_tool_call, 'tool_called_by_vulkan': internal_tool, 'tool_arguments_by_vulkan': internal_args, 'dispatcher_executed_internal_tool': True, 'wrapper_generated_by': '3572 deterministic broker mapping', 'vulkan_wrapped_dispatcher_result': False}, 'broker_pipeline_contract': '30B/OpenWebUI -> 3571 bridge -> 3572 broker -> 11435 selects internal L -> 3572 dispatcher executes L -> 3572 deterministic wrapper maps L result as public X -> 3571 -> 30B'}
