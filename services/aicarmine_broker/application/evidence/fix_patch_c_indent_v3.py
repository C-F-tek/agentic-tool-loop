...
old_code = '''        violations.append("repo_analysis_final_follow_up_invitation_instead_of_answer")
...
new_code = '''        violations.append("repo_analysis_final_follow_up_invitation_instead_of_answer")
    if red_flags.get("speculative_terms"):
        read_note_count = metrics.get("read_note_count", 0) or len(metrics.get("verified_content_reads", [])) or 0
        if read_note_count > 0:'''
