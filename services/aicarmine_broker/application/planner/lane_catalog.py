...
CONTROL_LANE_SPECS: tuple[ControlLaneSpec, ...] = (
    ...
    ControlLaneSpec(
        lane_id="02 planner.cuda_rewrite",
        kind="ai",
        phase="rewrite",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="04 judge.final_quality",
        kind="ai",
        phase="judge",
        runtime_scope="loop",
    ...
    ControlLaneSpec(
        lane_id="06 preplanner.semantic_query",
        kind="ai",
        phase="preplan",
        runtime_scope="loop",
        authority="advisory_only",
    ...
    ControlLaneSpec(
        lane_id="07 planner.native_tool_batch",
        kind="hybrid",
        phase="plan",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="08 planner.guided_terminal_final_quality",
        kind="hybrid",
        phase="terminal",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="09 routing.evidence_gap",
        kind="hybrid",
        phase="route",
        runtime_scope="loop",
        authority="advisory_only",
    ...
    ControlLaneSpec(
        lane_id="10 planner.incomprehensible_retry",
        kind="hybrid",
        phase="retry",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="11 planner.native_protocol_recovery",
        kind="hybrid",
        phase="retry",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="12 orientation.initial",
        kind="ai_candidate",
        phase="orientation",
        runtime_scope="loop",
    ...
    ControlLaneSpec(
        lane_id="13 orientation.area_expansion",
        kind="ai_candidate",
        phase="orientation",
        runtime_scope="loop",
        authority="bounded_selection",
    ...
    ControlLaneSpec(
        lane_id="14 candidate_actions.ranking",
        kind="ai_candidate",
        phase="rank",
        runtime_scope="loop",
    ...
    ControlLaneSpec(
        lane_id="15 planner.max_step_terminal_synthesis",
        kind="ai_candidate",
        phase="terminal",
        runtime_scope="loop",
        authority="proposal_only",
    ...
    ControlLaneSpec(
        lane_id="16 coverage.interpretation",
        kind="hybrid",
        phase="coverage",
        runtime_scope="loop",
    ...
    ControlLaneSpec(
        lane_id="18 quality.deterministic_floor",
        kind="deterministic",
        phase="judge",
        runtime_scope="loop",
        authority="deterministic_enforcement",
    ...
    ControlLaneSpec(
        lane_id="19 audit.specialist_route",
        kind="deterministic",
        phase="audit",
        runtime_scope="loop",
        authority="deterministic_enforcement",
    ...
    ControlLaneSpec(
        lane_id="24 guard.repeated_rejection_breaker",
        kind="deterministic",
        phase="guard",
        runtime_scope="loop",
        authority="deterministic_enforcement",
    ...
    ControlLaneSpec(
        lane_id="25 lifecycle.terminal_status",
        kind="deterministic",
        phase="lifecycle",
        runtime_scope="loop",
        authority="deterministic_enforcement",
    ...
    ControlLaneSpec(
        lane_id="28 boundary.internal_tool_selector",
        kind="boundary",
        phase="select",
...
CONTROL_LANE_BY_ID: Mapping[str, ControlLaneSpec] = (
...
def validate_control_lane_catalog() -> list[str]:
    ...
    for spec in CONTROL_LANE_SPECS:
        ...
        if spec.may_execute_tools and spec.lane_id != "23 dispatch.tool":
            errors.append(
                f"may_execute_tools=true only allowed for dispatch.tool, got {spec.lane_id}"
            )
        if spec.kind == "deterministic" and spec.affects_control_flow:
            # Deterministic lanes should not affect control flow
            pass  # This is actually allowed per spec
        if spec.kind == "sidecar" and spec.affects_control_flow:
            errors.append(
                f"Sidecar lane {spec.lane_id} should not affect control flow"
            )
