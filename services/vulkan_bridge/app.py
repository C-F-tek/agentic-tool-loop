...
def _legacy_agentic_v2_compact_context_for_openwebui(ctx):
    ...
    if isinstance(planner, dict):
        keep["planner"] = {
            ...
            "history_count": planner.get("history_count"),
...
def _effective_wait_seconds(payload: dict[str, Any]) -> int:
    ...
    if return_mode in {"background", "async", "fire_and_forget"}:
        return 0
    try:
        return int(payload.get("wait_seconds") or os.environ.get("AICARMINE_AGENT_RETURN_WAIT_SECONDS") or 900)
    except Exception:
        return 900
...
def _build_agent_payload(raw_payload: dict[str, Any], public_tool_x: str = "helper_for_all") -> dict[str, Any]:
    ...
    return {
        ...
        "return_mode": raw_payload.get("return_mode", "wait") if isinstance(raw_payload.get("return_mode", "wait"), str) else "wait",
...
def _agentic_v9_build_observation_object(decoded):
    ...
    observation = {
        ...
        "planner": _agentic_v9_clean({
            ...
            "history_count": _agentic_v9_as_dict(planner).get("history_count") or len(calls),
...
def _agentic_v9_priority_item_from_artifact(row):
    ...
    if kind == "repo_read":
        ...
        return _agentic_v9_clean({
            ...
            "content_sha256": _agentic_v9_hashlib.sha256(content.encode("utf-8", errors="replace")).hexdigest(),
            "chars": len(content),
...
def _agentic_v9_build_payload_index_for_30b(priority_evidence, tool_context, *, completed):
                ...
                suggestions_only.extend([
                    {
                        ...
                        "role": "metadata di review, non motivo per richiamare il tool",
                    },
                    {
...
def _agentic_v9_build_outer_tool_text(decoded, answer):
    observation = _agentic_v9_build_observation_object(decoded)
    lines = []

    _agentic_v9_append_successful_tool_index(lines, observation)

    lines.append("")
    _agentic_v9_append_successful_tool_evidence(lines, observation)

    limits = _agentic_v9_successful_tool_limits(observation)
    lines.extend(["", "LIMITI ESPLICITI:"])
    if limits:
        lines.extend(limits)
    else:
        lines.append("- nessun limite esplicito dichiarato dai tool riusciti.")
    return "\n".join(lines).strip() + "\n"
