...
| `codex_bridge/repo_code_change_set.py` | Content-addressed change-set owner for Codex/Cline unified diffs. | Persists bounded validated diff bytes and repository/commit/preimage metadata under ignored repo state, then resolves integrity-checked IDs. | No arbitrary paths, whole-file transport, binary/delete/rename patches or writes outside ignored state. |
| `codex_bridge/repo_code_mcp_server.py` | Incubating Codex repo-code MCP server for candidate code edit tools. | Imports broker proposal/check helpers and propagates exact or multi-file unified-diff changes through `change_set_id`. | Must stay isolated from stable MCPs; source writes require explicit `allow_source_write=true`. |
