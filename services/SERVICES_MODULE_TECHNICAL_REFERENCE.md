...
| `codex_bridge/repo_mcp_common.py` | Shared stdio/JSON-RPC, argument validation, health and root-selection helpers for deterministic Codex repo MCP servers. | Resolves `AICARMINE_CODEX_MCP_REPO_ROOT`/Codex env/cwd, then rewrites only the MCP process' `AICARMINE_LAB_REPO` before broker-tool imports. | Do not let an inherited OpenWebUI lab shadow override the Codex-selected root. |
...
| `codex_bridge/repo_code_change_set.py` | Content-addressed structured-edit and change-set owner for Codex/Cline. | Reads UTF-8 preimages, applies exact bounded operations, generates canonical LF diffs, persists repository/commit/preimage metadata and resolves integrity-checked IDs. | Maximum 2 MiB/100 files; no arbitrary paths, regex, binary/delete/rename operations or source writes during proposal. |
| `codex_bridge/repo_code_mcp_server.py` | Incubating Codex repo-code MCP server for candidate code edit tools. | Prefers multi-file `structured_edit`, generates diffs through the change-set owner, and preserves unified-diff/change-set/exact legacy compatibility. | Must keep the same five tool names, reject mixed authoring modes and require `allow_source_write=true` for source changes. |
| `codex_bridge/ops_mcp_server.py` | Incubating Codex ops MCP server for read-only service-state inspection. | Reads process, TCP listener and repo-local log state. | Must not become a command runner, must not call HTTP health routes and must redact command-line secrets. |
...
| `codex_bridge/agentic_loop_client_mcp_server.py` | Explicit Codex MCP client for a dedicated canonical broker loop, defaulting to 3579. | Can ensure a dedicated broker bound to the Codex repo root and can explicitly ensure the repo-local OVMS/BGE reranker on 3550 before broker/run calls. | Requires confirmation tokens for reranker startup, broker startup and loop calls; never use 3571/3572/11434/11435 as this client target. |
| `codex_bridge/local_subagent_mcp_server.py` | Codex local subagent facade over the dedicated 3579 agentic-loop client. | Delegates bounded read-only work to `agentic_loop_client_mcp_server.py`; does not call Ollama directly and does not expose local repo/RAG/git tool handlers. | Must stay a facade over the real broker planner/controller/validator loop, not a parallel subagent implementation. |
