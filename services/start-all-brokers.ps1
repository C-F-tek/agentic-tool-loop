...
# Start Vulkan Tool Broker on port 3579
if (-not (Test-Port 3579)) {
    Write-Host "[INFO] Starting Vulkan Tool Broker on port 3579..." -ForegroundColor Yellow
    Set-Location "C:\Users\sanit\\agentic-tool-loop\services"
    Start-Process -FilePath "python" -ArgumentList @("-m", "uvicorn", "services.aicarmine_vulkan_tool_broker:app", "--host", "127.0.0.1", "--port", "3579") -WorkingDirectory "C:\Users\sanit\\agentic-tool-loop\services" -WindowStyle Hidden
...
if (-not (Test-Port 3579)) {
    ...
    Set-Location "C:\Users\sanit\\agentic-tool-loop\services"
    Start-Process -FilePath "python" -ArgumentList @("-m", "uvicorn", "services.codex_bridge.agentic_loop_client_mcp_server:app", "--host", "127.0.0.1", "--port", "3579") -WorkingDirectory "C:\Users\sanit\\agentic-tool-loop\services" -WindowStyle Hidden
    Start-Sleep -Seconds 3
    Set-Location "C:\Users\sanit\\agentic-tool-loop"
