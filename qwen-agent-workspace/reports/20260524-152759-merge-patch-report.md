# Qwen Agent Merge Patch Report

- Source diff: `C:\Users\carmi\AI\lab-patches\master-working-tree-to-lab.diff`
- Patch artifact: `C:\Users\carmi\AI\qwen-agent-workspace\patches\merge-from-main.patch`
- Report JSON: `C:\Users\carmi\AI\qwen-agent-workspace\reports\20260524-152759-merge-patch-report.json`
- Apply requested: `False`

## Touched files

- `Tools/validation/TOOL_CONTEXT.md`
- `Tools/validation/dispatch.py`
- `Tools/validation/provider_mesh/TOOL_CONTEXT.md`
- `Tools/validation/runtime_tool/run_file_backed_transport_contract_smoke/cli.py`
- `docs/IA_UNIVERSE_MODEL_TO_CODE_MAP.md`
- `ia_carmine/CONTEXT_INDEX.md`
- `ia_carmine/TOOL_CONTEXT.md`
- `ia_carmine/_shared/agent_runtime_tool_broker_execution.py`
- `ia_carmine/_shared/file_backed_transport.py`
- `ia_carmine/_shared/provider_ollama_report.py`
- `ia_carmine/_shared/provider_replight.py`
- `ia_carmine/_shared/provider_tool_loop.py`
- `ia_carmine/_shared/provider_work_verification.py`
- `ia_carmine/context/heap_context_memory_reload/dynamic_gpu1_context.py`
- `ia_carmine/dispatch.py`
- `ia_carmine/product/heap_final_proposals/artifacts.py`
- `ia_carmine/product/heap_final_proposals/cli.py`
- `ia_carmine/product/heap_final_proposals/rendering.py`
- `ia_carmine/providers/provider_mesh/TOOL_CONTEXT.md`
- `ia_carmine/providers/provider_mesh/local_provider_probe/cli.py`
- `ia_carmine/providers/provider_mesh/ollama_gpu0_peer_report/cli.py`
- `ia_carmine/providers/provider_mesh/openvino_npu_model_probe/cli.py`
- `ia_carmine/runtime/heap_context_closure/TOOL_CONTEXT.md`
- `ia_carmine/runtime/heap_context_closure/commands.py`
- `ia_carmine/runtime/heap_context_closure/launcher.py`
- `ia_carmine/runtime/heap_gate/gpu1_closure_packet.py`
- `ia_carmine/runtime/heap_gate/provider_block_contract.py`
- `ia_carmine/runtime/heap_gate/provider_commands.py`
- `ia_carmine/runtime/heap_gate/provider_context.py`
- `ia_carmine/runtime/provider_runtime_blackboard/broker_bridge/cli.py`
- `ia_carmine/runtime/runtime_tool/agent_broker/cli.py`
- `ia_carmine/runtime/runtime_tool/broker/executor.py`

## Check result

- check_passed: `False`
- stderr: ``
