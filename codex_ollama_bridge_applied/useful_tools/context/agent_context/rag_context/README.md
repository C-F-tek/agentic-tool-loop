...
`rag_context/` contains RAG context ingestion, retrieval, chunking, embedding,
store and context-pack helpers.
...
- [ingest_repo_cli.py](ingest_repo_cli.py), [query_context_cli.py](query_context_cli.py), [build_context_pack_cli.py](build_context_pack_cli.py)
  - CLI entrypoints for ingestion, querying and context-pack generation.
- [chunking.py](chunking.py), [embedding.py](embedding.py), [retrieval.py](retrieval.py), [store.py](store.py)
  - Chunking, embedding, retrieval and storage helpers.
- [schema.py](schema.py), [context_pack.py](context_pack.py), [unified_pack.py](unified_pack.py)
  - Schema and pack composition helpers.
- [repo_files.py](repo_files.py), [index_status.py](index_status.py), [common.py](common.py)
  - Repo file selection, index status and shared utilities.
